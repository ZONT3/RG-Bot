    ...
    public static ArrayList<GM> retrieve() {
        ...
        try (Connection connection = DriverManager.getConnection(Globals.dbConnection);
            ...
            ResultSet resultSet = st.executeQuery(
                    "SELECT p_uid, p_id_dis, p_name, p_lastupd, p_roles FROM profiles " +
                        "WHERE p_roles != '[]' " +
                        "ORDER BY p_lastupd"
            );
    ...
    public static class Msg {
        public static MessageEmbed statusGMsInitial() {
            return new EmbedBuilder()
                    .setTitle(STR.getString("comms.gms.get.title"))
        ...
        public static MessageEmbed gmListShort(List<GM> gms,
            ...
            if (splitIndex > 0) {
                ...
                builder.appendDescription(String.format(
                        ...
                        Tools.countGMs(splitIndex), sleep, anger, warn, "//"));
        ...
        public static ArrayList<EmbedBuilder> gmList(List<GM> gms,
            ...
            for (GM gm: gms)
                ZDSBMessages.appendDescriptionSplit(buildField(gm, s, n, a, o), list);
            return list;
        }
        ...
        private static EmbedBuilder prepareGmList(List<GM> gms, boolean hide) {
            ...
            builder.setTitle(STR.getString("comms.gms.get.title"));
            for (GM gm: gms)
                if (gm.armaname == null) gm.armaname = STR.getString("comms.gms.get.unknown");
        ...
        private static String buildField(GM gm, boolean s, boolean n, boolean a, boolean o) {
            String memberMention = gm.userid > 0 ? String.format("<@%d>", gm.userid)
                    : STR.getString("comms.gms.get.unknown.person");
            StringBuilder field = new StringBuilder(memberMention).append('\n');

            if (s) field.append(String.format(STR.getString("comms.gms.get.steamid"), gm.steamid64)).append("\n");
            if (n) field.append(String.format(STR.getString("comms.gms.get.armaname"), gm.armaname)).append("\n");
            if (a) field.append(String.format(STR.getString("comms.gms.get.assigned"), getAssigned(gm))).append("\n");
        ...
        private static String getAssigned(GM gm) {
            ...
            if (assignedDate == null) return STR.getString("comms.gms.get.unknown");
        ...
        private static String getOnline(GM gm) {
            long lastLogin = gm.lastlogin;
            if (lastLogin == 0) return STR.getString("comms.gms.get.lastlogin.unk");
            ...
            if (min < 2) return STR.getString("comms.gms.get.lastlogin.n");
            else return String.format(STR.getString("comms.gms.get.lastlogin"), day, hr % 24, min % 60) + emo;
        }
